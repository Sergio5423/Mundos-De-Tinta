...
const RegistrarEmpleado = () => {
  ...
  const getNombres = async () => {
    const allNombres = await fetch(
      "http://localhost:3000/departamentos/nombres"
    );
  ...
  const handlerFormSubmit = async (e) => {
    e.preventDefault();
    if (
      validateDigitsCedula(dataForm.cedula) &&
      validateDigitsDireccion(dataForm.direccion) &&
      validateDigitsNombre(dataForm.nombre) &&
      validateDigitsEmail(dataForm.email) &&
      validateDigitsTelefono(dataForm.telefono) &&
      validateDigitsContraseña(dataForm.contraseña) &&      
      validateDepartamento(opcionSeleccionada)
    ) {
  ...
  const validateDigitsCedula = (value) => {
    const minDigits = 5;
    const maxDigits = 15;
    if (value.length < minDigits || value.length > maxDigits) {
      alert(
        `El campo cedula debe tener entre ${minDigits} y ${maxDigits} dígitos.`
      );
      return false;
    }
    return true;
  ...
  const validateDigitsDireccion = (value) => {
    const minDigits = 10;
    const maxDigits = 50;
    if (value.length < minDigits || value.length > maxDigits) {
      alert(
        `El campo direccion debe tener entre ${minDigits} y ${maxDigits} dígitos.`
      );
      return false;
    }
    return true;
  ...
  const validateDigitsEmail = (value) => {
    const minDigits = 10;
    const maxDigits = 60;
    if (value.length < minDigits || value.length > maxDigits) {
      alert(
        `El campo email debe tener entre ${minDigits} y ${maxDigits} dígitos.`
      );
      return false;
    }
    return true;
  ...
  const validateDigitsTelefono = (value) => {
    const minDigits = 8;
    const maxDigits = 12;
    if (value.length < minDigits || value.length > maxDigits) {
      alert(
        `El campo telefono debe tener entre ${minDigits} y ${maxDigits} dígitos.`
      );
      return false;
    }
    return true;
  ...
  const validateDigitsContraseña = (value) => {
    const minDigits = 8;
    const maxDigits = 24;
    if (value.length < minDigits || value.length > maxDigits) {
      alert(
        `El campo contraseña debe tener entre ${minDigits} y ${maxDigits} dígitos.`
      );
      return false;
    }
    return true;
